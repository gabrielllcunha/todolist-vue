...
<template>
  <div class="items-wrapper">
    <div v-if="isLoading" :style="{ marginTop: '1rem' }">
      <el-progress :percentage="100" :format="() => 'Carregando...'" :indeterminate="true" />
    </div>
    <div v-else>
      <div v-for="(item, index) in filteredItems" :key="index" class="item">
        <div class="item-wrapper">
          <el-icon
            ...
            :style="{
              color:
                hoveredObject.type === 'check' &&
                hoveredObject.id === item.id &&
          ...
          <div class="item-content">
            <div class="item-text">
              <span
                v-if="editingItem.id !== item.id || editingItem.field !== 'title'"
                class="item-title"
                :class="{ dashed: item.status === 'completed' }"
                @click="item.status === 'incomplete' ? setEditingItem(item.id, 'title') : null"
                @mouseover="
                  item.status === 'incomplete' ? setHoveredObject('title', item.id) : null
                "
                @mouseleave="setHoveredObject(null, null)"
                :style="{
                  cursor: props.activeTab === 'completed' ? 'default' : 'pointer',
                  backgroundColor:
                    hoveredObject.type === 'title' && hoveredObject.id === item.id
                      ? '#FFFFFF1A'
                      : 'transparent'
                }"
              >
              ...
              <span
                ...
                :class="{ dashed: item.status === 'completed' }"
                @click="
                  item.status === 'incomplete' ? setEditingItem(item.id, 'description') : null
                "
                @mouseover="
                  item.status === 'incomplete' ? setHoveredObject('description', item.id) : null
                "
                @mouseleave="setHoveredObject(null, null)"
                :style="{
                  ...
                  cursor: props.activeTab === 'completed' ? 'default' : 'pointer',
                  backgroundColor:
                    hoveredObject.type === 'description' && hoveredObject.id === item.id
                      ? '#FFFFFF1A'
                      : 'transparent'
                }"
              >
                {{
                  item.status === 'incomplete' && !item.description
            ...
            <div class="item-info">
              ...
              <el-icon
                ...
                :style="{
                  color:
                    hoveredObject.type === 'delete' && hoveredObject.id === item.id
                      ? '#FF4C4C'
                      : '#EBEBEBA3',
                  transition: 'color 0.3s ease'
                }"
